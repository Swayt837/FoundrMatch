...
This file used to also stub `emergentintegrations`, a private SDK that only existed
inside the Emergent platform and was imported at module scope by `ai_service` and
`premium` — without the stub no test could import a router. The app now runs on the
official `anthropic` and `stripe` packages, so the stub is gone and tests import the
real thing.
"""
import os
